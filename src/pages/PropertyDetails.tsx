        ...
        <section className="container mx-auto px-4 pb-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Owner Section - YouTube style */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.28 }}
              >
                <Link to={`/owner/${property.owner.id}`} className="group/owner flex items-center gap-3 py-3">
                  {/* Circle Avatar */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-medium group-hover/owner:text-primary transition-colors">{property.owner.name}</span>
                      {property.owner.verified && (
                        <CheckCircle2 className="h-3.5 w-3.5 text-muted-foreground" />
                      )}
                      <span className="text-xs text-muted-foreground">• {ownerTypeLabels[property.owner.type as OwnerType]}</span>
                    </div>
